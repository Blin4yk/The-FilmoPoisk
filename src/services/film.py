...
class FilmService:
    ...
    async def get_by_id(
        ...
        if permission_service:
                            ...
                            for fmt in [
                                ...
                                try:
                                    creation_date = datetime.strptime(
                                        creation_date_str.split('T')[0], fmt
                                    )
                                    break
                                except ValueError:
                                    continue
            ...
            can_access = await permission_service.can_access_new_film(
                user, creation_date
            )
            if not can_access:
                return None
    ...
    def _convert_to_film_detail(data: dict) -> FilmDetail:
        ...
        return FilmDetail(
            ...
            actors=[
                Person(id=person['id'], full_name=person['name'])
    ...
    def _convert_to_film_short(data: dict) -> FilmShort:
        ...
        return FilmShort(
            id=data['id'], title=data['title'], imdb_rating=data.get('imdb_rating')
        )
