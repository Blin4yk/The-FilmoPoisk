...
class TestFilmEndpoints:
    ...
    def test_get_film_details_not_found(self, api_client):
        ...
        assert (
            data['detail']
            == 'Фильм не найден или доступ запрещен. Новые фильмы требуют роль subscriber.'
        )
    ...
    def test_get_films_list_with_sorting(self, api_client):
        ...
        ratings = [
            film['imdb_rating'] for film in data if film['imdb_rating'] is not None
        ]
    ...
    def test_films_search_no_results(self, api_client):
        ...
        response = api_client.get(
            '/films/search/', params={'query': 'nonexistentmovie'}
        )
